...
import { Moon, Sun, Monitor, LogOut, BookOpen } from 'lucide-react';

export function UserMenu() {
  const { user, logout } = useAuth();
  const { theme, setTheme } = useTheme();
  const { startOnboarding, isActive } = useOnboarding();
  const { desktopMenuOpen, setDesktopMenuOpen } = useUserMenu();
  ...
  return (
    <DropdownMenu open={desktopMenuOpen} onOpenChange={setDesktopMenuOpen}>
      ...
      <DropdownMenuContent align="end" className="w-56" data-onboarding="user-menu-dropdown">
        <DropdownMenuLabel>
